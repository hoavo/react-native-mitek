...
- (void)configureSession
{
    ...
    if ([self.session canSetSessionPreset:self.sessionPreset] == YES) {
...
//    NSArray *supportedMetadataTypes = self.metaDataOutput.availableMetadataObjectTypes;
    ...
    if ([self.captureDevice isFocusPointOfInterestSupported] == YES)
    ...
    //    Consider adding these at some point when more devices support them.
    //    if ([self.captureDevice isSmoothAutoFocusSupported] == YES)
    //        [self.captureDevice setSmoothAutoFocusEnabled:NO];
    //
    //    if ([self.captureDevice isAutoFocusRangeRestrictionSupported] == YES)
    //        [self.captureDevice setAutoFocusRangeRestriction:NO];
    ...
    //NSLog(@"Updating session output connections");
    if (self.cameraOrientation == UIInterfaceOrientationLandscapeLeft)
    {
        NSArray<__kindof AVCaptureOutput *> *outputs = self.session.outputs;
        for (AVCaptureOutput *output in outputs)
    ...
    //NSLog(@"Finished updating session output connections. Time: %.4f seconds", [end timeIntervalSinceDate:start]);
